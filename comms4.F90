...
contains
  ...
  subroutine start_halo_update_4(size4, Array, tag, reqs)
    ...
    call MPI_Isend(Array, 1, halo_4_xup_send(size4), ip_xup, 0 + tag_offset, &
                   comm, reqs(1), ierr)
    call MPI_Irecv(Array, 1, halo_4_xdn_recv(size4), ip_xdn, 0 + tag_offset, &
                   comm, reqs(2), ierr)
    call MPI_Isend(Array, 1, halo_4_xdn_send(size4), ip_xdn, 1 + tag_offset, &
                   comm, reqs(3), ierr)
    call MPI_Irecv(Array, 1, halo_4_xup_recv(size4), ip_xup, 1 + tag_offset, &
                   comm, reqs(4), ierr)

    ! Start send and receive in y direction
    call MPI_Cart_Shift(comm, 1, 1, ip_ydn, ip_yup, ierr)
    call MPI_Isend(Array, 1, halo_4_yup_send(size4), ip_yup, 2 + tag_offset, &
                   comm, reqs(5), ierr)
    call MPI_Irecv(Array, 1, halo_4_ydn_recv(size4), ip_ydn, 2 + tag_offset, &
                   comm, reqs(6), ierr)
    call MPI_Isend(Array, 1, halo_4_ydn_send(size4), ip_ydn, 3 + tag_offset, &
                   comm, reqs(7), ierr)
    call MPI_Irecv(Array, 1, halo_4_yup_recv(size4), ip_yup, 3 + tag_offset, &
                   comm, reqs(8), ierr)

    ! Start send and receive in t direction
    call MPI_Cart_Shift(comm, 2, 1, ip_tdn, ip_tup, ierr)
    call MPI_Isend(Array, 1, halo_4_tup_send(size4), ip_tup, 4 + tag_offset, &
                   comm, reqs(9), ierr)
    call MPI_Irecv(Array, 1, halo_4_tdn_recv(size4), ip_tdn, 4 + tag_offset, &
                   comm, reqs(10), ierr)
    call MPI_Isend(Array, 1, halo_4_tdn_send(size4), ip_tdn, 5 + tag_offset, &
                   comm, reqs(11), ierr)
    call MPI_Irecv(Array, 1, halo_4_tup_recv(size4), ip_tup, 5 + tag_offset, &
                   comm, reqs(12), ierr)
  ...
  subroutine start_halo_update_4_real(size4, Array, tag, reqs)
    ...
    ! Start send and receive in x direction
    call MPI_Cart_Shift(comm, 0, 1, ip_xdn, ip_xup, ierr)
    call MPI_Isend(Array, 1, halo_4_real_xup_send(size4), ip_xup, 0 + tag_offset, &
                   comm, reqs(1), ierr)
    call MPI_Irecv(Array, 1, halo_4_real_xdn_recv(size4), ip_xdn, 0 + tag_offset, &
                   comm, reqs(2), ierr)
    call MPI_Isend(Array, 1, halo_4_real_xdn_send(size4), ip_xdn, 1 + tag_offset, &
                   comm, reqs(3), ierr)
    call MPI_Irecv(Array, 1, halo_4_real_xup_recv(size4), ip_xup, 1 + tag_offset, &
                   comm, reqs(4), ierr)

    ! Start send and receive in y direction
    call MPI_Cart_Shift(comm, 1, 1, ip_ydn, ip_yup, ierr)
    call MPI_Isend(Array, 1, halo_4_real_yup_send(size4), ip_yup, 2 + tag_offset, &
                   comm, reqs(5), ierr)
    call MPI_Irecv(Array, 1, halo_4_real_ydn_recv(size4), ip_ydn, 2 + tag_offset, &
                   comm, reqs(6), ierr)
    call MPI_Isend(Array, 1, halo_4_real_ydn_send(size4), ip_ydn, 3 + tag_offset, &
                   comm, reqs(7), ierr)
    call MPI_Irecv(Array, 1, halo_4_real_yup_recv(size4), ip_yup, 3 + tag_offset, &
                   comm, reqs(8), ierr)

    ! Start send and receive in t direction
    call MPI_Cart_Shift(comm, 2, 1, ip_tdn, ip_tup, ierr)
    call MPI_Isend(Array, 1, halo_4_real_tup_send(size4), ip_tup, 4 + tag_offset, &
                   comm, reqs(9), ierr)
    call MPI_Irecv(Array, 1, halo_4_real_tdn_recv(size4), ip_tdn, 4 + tag_offset, &
                   comm, reqs(10), ierr)
    call MPI_Isend(Array, 1, halo_4_real_tdn_send(size4), ip_tdn, 5 + tag_offset, &
                   comm, reqs(11), ierr)
    call MPI_Irecv(Array, 1, halo_4_real_tup_recv(size4), ip_tup, 5 + tag_offset, &
                   comm, reqs(12), ierr)
