module comms6
  ...
  integer :: halo_6_thirdup_send(4:4, 1:25), halo_6_thirddn_send(4:4, 1:25)
  integer :: halo_6_thirdup_recv(4:4, 1:25), halo_6_thirddn_recv(4:4, 1:25)
...
contains
  ...
  subroutine init_halo_types_6()
    ...
    do i5 = 1, nsize5
      do i6 = 1, nsize6
        ...
        call init_single_halo_type_6(-1, 1, size5(i5), size6(i6), &
                                     halo_6_thirddn_send(size5(i5), size6(i6)))
        call init_single_halo_type_6(-1, 0, size5(i5), size6(i6), &
                                     halo_6_thirddn_recv(size5(i5), size6(i6)))
        call init_single_halo_type_6(-1, kthird_l, size5(i5), size6(i6), &
                                     halo_6_thirdup_send(size5(i5), size6(i6)))
        call init_single_halo_type_6(-1, kthird_l + 1, size5(i5), size6(i6), &
                                     halo_6_thirdup_recv(size5(i5), size6(i6)))
      end do
    end do
  ...
  subroutine start_halo_update_6(size5, size6, Array, tag, reqs)
    ...
    call MPI_Isend(Array, 1, halo_6_xup_send(size5, size6), ip_xup, 0 + tag_offset, &
                   comm, reqs(1), ierr)
    call MPI_Irecv(Array, 1, halo_6_xdn_recv(size5, size6), ip_xdn, 0 + tag_offset, &
                   comm, reqs(2), ierr)
    call MPI_Isend(Array, 1, halo_6_xdn_send(size5, size6), ip_xdn, 1 + tag_offset, &
                   comm, reqs(3), ierr)
    call MPI_Irecv(Array, 1, halo_6_xup_recv(size5, size6), ip_xup, 1 + tag_offset, &
                   comm, reqs(4), ierr)

    ! Start send and receive in y direction
    call MPI_Cart_Shift(comm, 1, 1, ip_ydn, ip_yup, ierr)
    call MPI_Isend(Array, 1, halo_6_yup_send(size5, size6), ip_yup, 2 + tag_offset, &
                   comm, reqs(5), ierr)
    call MPI_Irecv(Array, 1, halo_6_ydn_recv(size5, size6), ip_ydn, 2 + tag_offset, &
                   comm, reqs(6), ierr)
    call MPI_Isend(Array, 1, halo_6_ydn_send(size5, size6), ip_ydn, 3 + tag_offset, &
                   comm, reqs(7), ierr)
    call MPI_Irecv(Array, 1, halo_6_yup_recv(size5, size6), ip_yup, 3 + tag_offset, &
                   comm, reqs(8), ierr)

    ! Start send and receive in t direction
    call MPI_Cart_Shift(comm, 2, 1, ip_tdn, ip_tup, ierr)
    call MPI_Isend(Array, 1, halo_6_tup_send(size5, size6), ip_tup, 4 + tag_offset, &
                   comm, reqs(9), ierr)
    call MPI_Irecv(Array, 1, halo_6_tdn_recv(size5, size6), ip_tdn, 4 + tag_offset, &
                   comm, reqs(10), ierr)
    call MPI_Isend(Array, 1, halo_6_tdn_send(size5, size6), ip_tdn, 5 + tag_offset, &
                   comm, reqs(11), ierr)
    call MPI_Irecv(Array, 1, halo_6_tup_recv(size5, size6), ip_tup, 5 + tag_offset, &
                   comm, reqs(12), ierr)

    ! Start send and receive in third direction
    call MPI_Cart_Shift(comm, 3, 1, ip_thirddn, ip_thirdup, ierr)
    call MPI_Isend(Array, 1, halo_6_thirdup_send(size5, size6), ip_thirdup, 6 + tag_offset, &
                   comm, reqs(13), ierr)
    call MPI_Irecv(Array, 1, halo_6_thirddn_recv(size5, size6), ip_thirddn, 6 + tag_offset, &
                   comm, reqs(14), ierr)
    call MPI_Isend(Array, 1, halo_6_thirddn_send(size5, size6), ip_thirddn, 7 + tag_offset, &
                   comm, reqs(15), ierr)
    call MPI_Irecv(Array, 1, halo_6_thirdup_recv(size5, size6), ip_thirdup, 7 + tag_offset, &
                   comm, reqs(16), ierr)
