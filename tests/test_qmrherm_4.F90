...
program test_qmrherm_4
  ...
  do idiag=1,ndiag
    ...
    ! relative error
    delta_Phi = abs(delta_Phi)**2/sum(real(xin*conjg(xin)))
    check_sum(delta_Phi, 1e-08, 'xout', sum_delta_Phi, MPI_Double_Precision, 'test_qmrherm_4_dp')
  ...
  do idiag=1,ndiag
    ...
    ! relative error
    delta_Phi = abs(delta_Phi)**2/sum(real(xin*conjg(xin)))
    check_sum(delta_Phi, 1e-07, 'xout', sum_delta_Phi, MPI_Double_Precision, 'test_qmrherm_4_sp')
