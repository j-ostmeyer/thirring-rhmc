     ...
     do it=1,ksizet_l
        do iy=1,ksizey_l
           do ix=1,ksizex_l
              do ithird=1,kthird
                 i = i + 1
                 test_array(ithird, ix, iy, it, i5) = cmplx(i, pid(ip_x, ip_y, ip_t))
              end do
           end do
        end do
     end do
...
#else
  call complete_halo_update_5(4, test_array)
#endif

! Check output
  if (real(test_array(1,1,1,1,1)) .ne. real(test_array(1,ksizex_l+1,1,1,1)) .or. &
       nint(aimag(test_array(1,ksizex_l+1,1,1,1))) &
  ...
  if (real(test_array(1,1,1,1,1)) .ne. real(test_array(1,1,ksizey_l+1,1,1)) .or. &
       nint(aimag(test_array(1,1,ksizey_l+1,1,1))) &
  ...
  if (real(test_array(1,1,1,1,1)) .ne. real(test_array(1,1,1,ksizet_l+1,1)) .or. &
       nint(aimag(test_array(1,1,1,ksizet_l+1,1))) &
  ...
  if (real(test_array(1,ksizex_l,1,1,1)) .ne. real(test_array(1,0,1,1,1)) .or. &
       nint(aimag(test_array(1,0,1,1,1))) &
       .ne. pid(modulo(ip_x-1, np_x), ip_y, ip_t)) then
  ...
  if (real(test_array(1,1,ksizey_l,1,1)) .ne. real(test_array(1,1,0,1,1)) .or. &
       nint(aimag(test_array(1,1,0,1,1))) &
       .ne. pid(ip_x, modulo(ip_y-1, np_y), ip_t)) then
  ...
  if (real(test_array(1,1,1,ksizet_l,1)) .ne. real(test_array(1,1,1,0,1)) .or. &
       nint(aimag(test_array(1,1,1,0,1))) &
       .ne. pid(ip_x, ip_y, modulo(ip_t-1, np_t))) then
