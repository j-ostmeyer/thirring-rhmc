...
program test_qmrherm_4
  ...
  do idiag = 1, ndiag
    ...
    check_sum(delta_Phi_Re, 2.6e-14, 'xout', sum_delta_Phi, MPI_Double_Precision, 'test_qmrherm_4_dp')
  enddo
  ...
  do idiag = 1, ndiag
    ...
    ! relative error
    delta_Phi_Re = abs(delta_Phi)**2/sum(abs(xout(1:kthird_l, 1:ksizex_l, 1:ksizey_l, 1:ksizet_l, :)**2))
    check_sum(delta_Phi_Re, 3.1e-9, 'xout', sum_delta_Phi, MPI_Double_Precision, 'test_qmrherm_4_sp')
