...
! initialise function parameters
      ...
      complex*16 dSdpi_ref(ksize, ksize, ksizet, 3)
      complex*16 R(kthird,0:ksize+1, 0:ksize+1, 0:ksizet+1, 4)
      complex*16 diff(ksize, ksize, ksizet, 3)
            ...
            do iy = 1,ksize
               do ix = 1,ksize
                  do ithird = 1,kthird
                     ...
                     X2(kthird, ix, iy, it, j) = 0.5 * exp(1.0) * exp(iunit * idx * tau / idxmax)
                  enddo
               enddo
            enddo
         enddo
         ...
         do it = 1,ksizet
            do iy = 1,ksize
               do ix = 1,ksize
                  ...
                  dSdpi(ix, iy, it, j) = 1.23 * exp(iunit * idx * tau / idxmax)
               enddo
            enddo
         enddo
      enddo
