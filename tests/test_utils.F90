...
subroutine generate_starting_state_Phi_and_X(Phi, reqs_Phi, u , X, reqs_X)
    implicit none
    ! Required inputs
    complex(dp), intent(inout) :: Phi(kthird_l, 0:ksizex_l + 1, 0:ksizey_l + 1, 0:ksizet_l + 1, 4)
    complex(dp), intent(inout) :: u(0:ksizex_l + 1, 0:ksizey_l + 1, 0:ksizet_l + 1, 3)
    complex(dp), intent(inout) :: X(kthird_l, 0:ksizex_l + 1, 0:ksizey_l + 1, 0:ksizet_l + 1, 4)
...
subroutine generate_starting_state(Phi, reqs_Phi, u, R, reqs_R, X, reqs_X, dSdpi_ref, Phi0_orig)
    ...
    ! Optional inputs
    complex(dp), intent(inout), optional :: R(kthird_l, 0:ksizex_l + 1, 0:ksizey_l + 1, 0:ksizet_l + 1, 4)
    complex(dp), intent(inout), optional :: X(kthird_l , 0:ksizex_l + 1, 0:ksizey_l + 1, 0:ksizet_l + 1, 4)
    complex(dp), intent(inout), optional :: Phi0_orig(kthird_l, 0:ksizex_l + 1, 0:ksizey_l + 1, 0:ksizet_l + 1, 4, 25)
