...
program test_qmrherm_0
  ...
  ! X2, reqs_X2 and dSdpi_ref are passed here simply to reduce the complexity of generate_starting_state
  ...
  if (generate) then
#ifdef MPI
    write_file(X(1:kthird_l, 1:ksizex_l, 1:ksizey_l, 1:ksizet_l, :), 'test_qmrherm_0_x.dat', MPI_Double_Complex)
