...
!  ! check output
  if (generate) then
    write_file(Phi(:, 1:ksizex_l, 1:ksizey_l, 1:ksizet_l, :), 'test_dslash_5.dat', MPI_Double_Complex)
  else
    read_file(Phiref, 'test_dslash_5.dat', MPI_Double_Complex)

    diff = Phi(:, 1:ksizex_l, 1:ksizey_l, 1:ksizet_l, :) - Phiref
    check_max(diff, 1e-11, 'Phi', max_diff, MPI_Double_Precision, 'test_dslash_5')
    check_sum(diff, 1e-11, 'Phi', sum_diff, MPI_Double_Complex, 'test_dslash_5')
  end if
#ifdef MPI
...
subroutine dslash(phi, r, u, am, imass)
  ...
  do ipt = -1, 1
    do ipy = -1, 1
      do ipx = -1, 1
        ichunk = (/ipx, ipy, ipt/)
        call dslash_split(Phi, R, u, am, imass, ichunk, border_partitions_cube,&
